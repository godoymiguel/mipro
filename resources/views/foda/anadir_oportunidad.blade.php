                ...
                <div class="card-body">
                    ...
                    @endif	
						...
						<div>								
							<table class="table">
							  <tr>
								<th scope="tg-yw4l">Oportunidades<br>
									<div class="form-group row">								 
										<textarea id="oportunidad" placeholder="Inserte la oportunidad"  cols="31" rows="7" class="form-control{{ $errors->has('oportunidad') ? ' is-invalid' : '' }}" name="oportunidad" required autofocus {{$method == 'show' ? 'disabled' : null }}>{{ old('oportunidad', $foda->oportunidad) }}</textarea>
											@if ($errors->has('oportunidad'))
												<span class="invalid-feedback">
													<strong>{{ $errors->first('oportunidad') }}</strong>
												</span>
											@endif
									</div>
								</th>
